...
SWING_LOOKBACK = 3 # For swing high/low detection
...
ICT_ASIAN_RANGE = {
    'start': time(0, 0),   # Midnight NY time
    'end': time(5, 0)      # 5 AM NY time
}

ICT_LONDON_KILLZONE = {
    'start': time(3, 0),   # 3 AM NY time (when Judas Swing forms)
    'end': time(5, 0)      # 5 AM NY time
}

ICT_NEWYORK_KILLZONE = {
    'start': time(7, 0),   # 7 AM NY time
    'end': time(10, 0)     # 10 AM NY time
}
...
# Entry Configuration
REQUIRE_KILLZONE = False  # Only trade during kill zones
...
RETRACEMENT_THRESHOLD_PERCENT = 25.0  # Minimum retracement for entry
