...
class ICTTradingBot:
    ...
    def run_cycle(self):
        ...
        if require_killzone:
            ...
            # Check London Kill Zone (3-5 AM NY)
            if (config.ICT_LONDON_KILLZONE['start'] <= current_ny_time < 
                config.ICT_LONDON_KILLZONE['end']):
                in_any_killzone = True
                active_zone = "LONDON KILL ZONE"
                
            # Check New York Kill Zone (7-10 AM NY)
            elif (config.ICT_NEWYORK_KILLZONE['start'] <= current_ny_time < 
                config.ICT_NEWYORK_KILLZONE['end']):
                in_any_killzone = True
