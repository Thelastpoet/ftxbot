...
class IndicatorCalculator:
    ...
    def calculate_indicators(self, data, rsi_period=14, adx_period=14, atr_period=14,
        ...
        try:
            ...
            # Ichimoku Cloud
            df['ichimoku_tenkan_sen'] = (df['high'].rolling(window=ichimoku_tenkan_sen).max() + 
                                        df['low'].rolling(window=ichimoku_tenkan_sen).min()) / 2
            df['ichimoku_kijun_sen'] = (df['high'].rolling(window=ichimoku_kijun_sen).max() + 
                                    df['low'].rolling(window=ichimoku_kijun_sen).min()) / 2
            df['ichimoku_senkou_span_a'] = ((df['ichimoku_tenkan_sen'] + df['ichimoku_kijun_sen']) / 2).shift(ichimoku_kijun_sen)
            df['ichimoku_senkou_span_b'] = ((df['high'].rolling(window=ichimoku_senkou_span_b).max() + 
                                            df['low'].rolling(window=ichimoku_senkou_span_b).min()) / 2).shift(ichimoku_kijun_sen)
            ...
            max_period = max(
                ema_long_period,  # Longest EMA
                adx_period + dmi_period,  # ADX needs both periods
                macd_slowperiod + macd_signal_period,  # MACD needs both periods
                ichimoku_senkou_span_b + ichimoku_kijun_sen,  # Ichimoku cloud longest period
                chandelier_period,
                momentum_period
            )
    ...
    def calculate_chandelier_exit(self, data, period=22, multiplier=3):
        ...
        # Use the long exit for buy positions and short exit for sell positions
