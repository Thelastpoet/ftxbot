...
# Import the config file to access BROKER_TIMEZONE
import config # <--- STEP 1: ADD THIS IMPORT
...
class MarketDataProvider:
    """Unified market data provider for all timeframes."""
    
    def __init__(self, mt5_client: MetaTrader5Client):
        self.client = mt5_client
        
    def get_data(self, symbol, timeframe_str, count=1000):
        """
        [CORRECTED] Unified method to fetch market data for any timeframe.
        Applies timezone correction for broker server time.
        """
        if not self.client.is_connected():
            logger.error(f"MT5 not connected, cannot fetch {timeframe_str} data for {symbol}")
            return None
            
        try:
            timeframe = self.client.timeframe_to_mql(timeframe_str)
            ...
            if df.empty:
                logger.warning(f"No valid candles after filtering for {symbol} on {timeframe_str}")
                return None
            
            # --- START: TIMEZONE CORRECTION ---
            # 1. Convert timestamp to NAIVE datetime objects first (no timezone).
            df['time'] = pd.to_datetime(df['time'], unit='s')
            df.set_index('time', inplace=True)
            
            # 2. Localize the naive index to the broker's actual timezone.
            #    This correctly stamps the time with its source timezone (e.g., GMT+3).
            df.index = df.index.tz_localize(config.BROKER_TIMEZONE)
            
            # 3. Convert the now-correct timestamp to the universal UTC standard.
            #    All subsequent analysis in the bot will use this correct UTC time.
            ...
            logger.error(f"Invalid timeframe '{timeframe_str}': {e}")
            return None
        except Exception as e:
            logger.error(f"Error fetching {timeframe_str} data for {symbol}: {e}", exc_info=True)
            return None
...
class TradeExecutor:
    ...
    def place_market_order(self, symbol, order_type, volume, sl_price, tp_price, comment=""):
        ...
        type_filling = self.client.get_supported_filling_mode(symbol)
        ...
        if result is None:
            ...
            logger.error(f"{symbol}: Order send failed to be processed by MT5. Error: {error_code} - {error_message}")
            logger.error(f"Failed Request Details: {request}") # Log the request for debugging
            return None
...
class SymbolManager:
    ...
    def check_spread(self, symbol, max_spread_config):
        """
        [Corrected] Check if spread is acceptable using a pair-specific config.
        Returns raw spread value if OK, else None.
        """
        ticker = self.client.get_symbol_ticker(symbol)
        if not ticker:
            return None
        ...
        # Determine the max spread for this specific symbol
        max_points = max_spread_config.get(symbol, max_spread_config.get('DEFAULT', 30))
