...
class ICTTradingBot:
    ...
    def process_symbol(self, symbol):
        ...
        for tf, count in config.TIMEFRAME_CONFIG.items():
            df = self.market_provider.get_ohlc(symbol, tf, count)
            if df is None or len(df) < 50: # Use a smaller, generic check here
        ...
        signal, sl_price, tp_price, narrative = self.signal_generator.generate_signal(ohlc_dfs, symbol, current_spread)
