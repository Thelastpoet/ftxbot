...
class MarketDataProvider:
    ...
    def get_data(self, symbol, timeframe_str, count=1000):
        """
        Unified method to fetch market data for any timeframe.
        
        Args:
            symbol: Trading symbol (e.g., 'EURUSD')
            timeframe_str: Timeframe as string (e.g., 'M15', 'H4', 'D1')
        ...
        try:
            ...
            if rates is None:
                logger.error(f"No rates available for {symbol} on {timeframe_str} timeframe")
                return None
                
            if len(rates) == 0:
                logger.warning(f"Empty rates returned for {symbol} on {timeframe_str}")
                return None
            
            # Convert to DataFrame
            df = pd.DataFrame(rates)
            
            # Filter out zero volume candles (weekends/holidays)
            ...
            # Rename tick_volume to volume for forex trading
            df.rename(columns={'tick_volume': 'volume'}, inplace=True)
            
            # Return only essential OHLCV columns
            df = df[['open', 'high', 'low', 'close', 'volume']]
            
            logger.debug(f"Fetched {len(df)} {timeframe_str} candles for {symbol}")
            ...
            logger.error(f"Error fetching {timeframe_str} data for {symbol}: {e}")
            return None
