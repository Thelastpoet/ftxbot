...
class smc:
    ...
    @classmethod
    def swing_highs_lows(cls, ohlc: DataFrame, swing_length: int = 1) -> Series:
        """
        Swing Highs and Lows (Fractal Implementation)
        ...
        swing_length: int - The number of candles to check on each side of the pivot candle.
                        Default is 1, creating a 3-bar fractal (1 past, 1 pivot, 1 future).
    ...
    def bos_choch(cls, ohlc: DataFrame, swing_highs_lows: DataFrame, close_break: bool = True) -> DataFrame:
        ...
        - CHoCH: Potential reversal of the current trend.
        """
        swings = swing_highs_lows.dropna().copy()
        swings['index_pos'] = [ohlc.index.get_loc(idx) for idx in swings.index]
        
        bos_choch_data = []
        
        last_swing_high = None
        last_swing_low = None
        trend = 'unknown' # Can be 'bullish' or 'bearish'

        swing_points = swings.to_dict('records')

        for i in range(len(swing_points)):
            current_swing = swing_points[i]
            
            if current_swing['HighLow'] == 1: # It's a swing high
                if last_swing_high is None:
                    last_swing_high = current_swing
                    continue

                # If the new high is higher than the last high
                if current_swing['Level'] > last_swing_high['Level']:
                    if trend == 'bullish': # Break of Structure (Continuation)
                        bos_choch_data.append({
                            'index': last_swing_high['index_pos'],
                            'type': 'BOS',
                            'direction': 1,
                            'level': last_swing_high['Level']
                        })
                    elif trend == 'bearish': # Change of Character (Reversal)
                        bos_choch_data.append({
                            'index': last_swing_high['index_pos'],
                            'type': 'CHOCH',
                            'direction': 1,
            ...
            else: # It's a swing low
                if last_swing_low is None:
                    last_swing_low = current_swing
                    continue

                # If the new low is lower than the last low
                if current_swing['Level'] < last_swing_low['Level']:
                    if trend == 'bearish': # Break of Structure (Continuation)
                        bos_choch_data.append({
                            'index': last_swing_low['index_pos'],
                            'type': 'BOS',
                            'direction': -1,
                            'level': last_swing_low['Level']
                        })
                    elif trend == 'bullish': # Change of Character (Reversal)
                        bos_choch_data.append({
                            'index': last_swing_low['index_pos'],
                            'type': 'CHOCH',
                            'direction': -1,
        ...
        if not bos_choch_data:
            return pd.DataFrame(index=ohlc.index, columns=['BOS', 'CHOCH', 'Level', 'BrokenIndex'])

        result_df = pd.DataFrame(index=ohlc.index, columns=['BOS', 'CHOCH', 'Level', 'BrokenIndex'])
        
        for item in bos_choch_data:
            break_level = item['level']
            start_index = item['index'] + 1
            
            broken_index = None
            # Determine the price series to check for break
            price_series = ohlc['close'] if close_break else (ohlc['high'] if item['direction'] == 1 else ohlc['low'])

            for j in range(start_index, len(price_series)):
                if (item['direction'] == 1 and price_series.iloc[j] > break_level) or \
                   (item['direction'] == -1 and price_series.iloc[j] < break_level):
                    broken_index = j
                    break
            
            if broken_index:
                original_index = ohlc.index[item['index']]
                result_df.loc[original_index, item['type']] = item['direction']
                result_df.loc[original_index, 'Level'] = break_level
                result_df.loc[original_index, 'BrokenIndex'] = broken_index

        return result_df.astype({'BOS': 'float64', 'CHOCH': 'float64', 'Level': 'float64', 'BrokenIndex': 'float64'})
