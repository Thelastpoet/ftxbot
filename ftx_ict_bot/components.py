...
class MarketDataProvider:
    ...
    def get_data(self, symbol, timeframe_str, count=1000):
        """Unified method to fetch market data for any timeframe with timezone correction."""
        ...
        try:
            ...
            # Localize to broker timezone then convert to UTC
...
class TradeExecutor:
    ...
    def place_market_order(self, symbol, order_type, volume, sl_price, tp_price, comment=""):
        ...
        if result is None:
            ...
            logger.error(f"Failed Request Details: {request}")
            return None
...
class SymbolManager:
    ...
    def check_spread(self, symbol, max_spread_config):
        """Check if spread is acceptable using pair-specific config."""
        ticker = self.client.get_symbol_ticker(symbol)
        if not ticker:
            return None
