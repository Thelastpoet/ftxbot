...
class ICTTradingBot:
    ...
    def __init__(self):
        ...
        if getattr(config, 'ENABLE_POSITION_MANAGEMENT', False):
            self.position_manager = PositionManager(
                self.mt5_client, 
                config, 
                self.signal_generator.liquidity_detector,
                self.signal_generator
            )
            logger.info("ICT Position Manager enabled")
        else:
            self.position_manager = None
            logger.info("ICT Position Manager disabled")
    ...
    def process_symbol(self, symbol):
        ...
        if signal:
            ...
            if result:
                ...
                # Register position for advanced management
                if self.position_manager:
                    try:
                        # Create position management plan
                        plan = self.position_manager.create_position_plan(
                            symbol, signal, result.price, sl_price, tp_price, 
                            volume, narrative, ohlc_df
                        )
                        
                        # Register position for management
                        if self.position_manager.register_position(result.order, plan):
                            logger.info(f"{symbol}: Position registered for ICT management")
                        else:
                            logger.warning(f"{symbol}: Failed to register position for management")
                            
                    except Exception as e:
                        logger.error(f"{symbol}: Error setting up position management: {e}")
    ...
    def run_cycle(self):
        ...
        if self.position_manager:
            try:
                ...
                actions = self.position_manager.update_positions(self.market_provider)
                update_time = time.time() - start_time
                
                if actions['positions_updated'] > 0:
                    logger.info(f"Position management update took {update_time:.2f}s")
                    ...
                    if time.time() % 300 < 60:  # Every 5 minutes
                        ...
                        if summary['total_positions'] > 0:
                            logger.info(f"Position Summary: {summary['total_positions']} positions, "
                                    f"Total P/L: {summary['total_profit']:.2f}, "
                                    f"Risk-free: {summary['risk_free_positions']}")
        ...
        if self.position_manager:
            try:
                ...
                if summary['total_positions'] > 0:
                    ...
                    logger.info(f"  Active positions: {summary['total_positions']}")
                    logger.info(f"  Total unrealized P/L: {summary['total_profit']:.2f}")
                    logger.info(f"  Average R:R ratio: {summary['avg_rr_ratio']:.2f}")
                    logger.info(f"  Risk-free positions: {summary['risk_free_positions']}")
                    
                    # Force close all positions for clean shutdown
                    self.position_manager.force_close_all_positions("Bot shutdown")
                    
            except Exception as e:
                logger.error(f"Error in position management shutdown: {e}")
