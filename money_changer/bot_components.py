...
class MarketDataProvider:
    ...
    def get_ohlc(self, symbol, timeframe_str, count):
        ...
        if rates is None or len(rates) == 0:
            ...
            logger.warning("Please check the following in your MetaTrader 5 terminal:")
            logger.warning(f"1. Is the symbol name '{symbol}' correct? Check the Market Watch.")
            logger.warning(f"2. Does the terminal have sufficient historical data for this timeframe? Scroll back on the chart to check.")
            return pd.DataFrame()
    ...
    def get_ohlc_range(self, symbol, timeframe_str, from_date, to_date):
        ...
        rates = mt5.copy_rates_range(symbol, timeframe_mql, from_date, to_date)

        if rates is None or len(rates) == 0:
            logger.warning(f"No rates returned for {symbol} on {timeframe_str} in the specified date range.")
            logger.warning("Please check the following in your MetaTrader 5 terminal:")
            logger.warning(f"1. Is the symbol name '{symbol}' correct? Check the Market Watch.")
            logger.warning(f"2. Does the terminal have sufficient historical data for this timeframe? Scroll back on the chart to check.")
            return pd.DataFrame()
        ...
        logger.debug(f"{symbol}: Fetched {len(ohlc_df)} candles for range {from_date} to {to_date}")
        return ohlc_df[['open', 'high', 'low', 'close', 'volume']]
