...
from trade_exit_manager import TradeExitManager
...
class AMDAnalyzer:
    ...
    def identify_amd_context(self, df, current_time, current_session, market_structure):
        ...
        if not asian_range and current_time.hour >= 9:
        ...
        # MANIPULATION PHASE (London Session: 09:00-13:00 UTC)
            ...
            # --- START OF THE FIX ---
            if asian_range:
                # This is the original logic, which is correct *if* we have the range.
                ...
                # NEW: If we are in the London session but asian_range is not ready,
                # we are in a specific waiting state.
                amd_context['phase'] = 'AWAITING_ASIAN_RANGE'
                amd_context['setup_bias'] = 'avoid_trades' # Explicitly avoid trading
            # --- END OF THE FIX ---
        ...
        # Final check to ensure phase is never None
        if amd_context['phase'] is None:
            amd_context['phase'] = 'UNDEFINED'
...
class AdaptiveTradeManager:
    ...
    def set_exit_manager(self, exit_manager):
        """Set exit manager after initialization to avoid circular dependency"""
            ...
            for setup in scored_setups:
                ...
                if amd['phase'] == 'ACCUMULATION':
                    ...
                    if amd['setup_bias'] and setup['direction'] == amd['setup_bias']:
                        ...
                        amd_adjustment -= 0.35
                elif amd['phase'] == 'AWAITING_ASIAN_RANGE':
                    # If we are waiting for the Asian Range, no setup is valid yet.
                    # Apply a massive penalty to ensure score is too low to trade.
                    amd_adjustment = -1.0
                    if 'confluence_notes' in setup:
                        setup['confluence_notes'].append("INVALID:Awaiting_Asian_Range")
    ...
    def execute_setup(self, setup, market_structure):
        ...
        try:
            ...
            # --- FIX #2: Check for invalid stop calculation ---
            # This prevents a crash if calculate_dynamic_stops returns (None, None)
            # because the trade setup was deemed invalid during calculation.
            if stop_distance is None or take_profit_distance is None:
                # The reason for failure is logged inside calculate_dynamic_stops
                logging.error(f"Failed to calculate valid stop/TP for {setup['type']} on {symbol}. Aborting trade.")
                return False, "Invalid stop/TP calculation"
            ...
            # --- FIX #1: Safely access regime parameters to prevent the 'NoneType' error ---
            regime_params = market_structure.get('regime_params')
            if not isinstance(regime_params, dict):
                logging.error(f"Regime parameters are missing or invalid in market structure for {symbol}. Cannot size position.")
                return False, "Invalid regime parameters"

            risk_mult = regime_params.get('risk_multiplier')
            if risk_mult is None:
                logging.warning(f"Risk multiplier not found or is None in regime_params for {symbol}, defaulting to 1.0.")
                risk_mult = 1.0
            ...
            # Store comprehensive context before placing order
            entry_context = {
                'setup': setup,
                ...
                'lot_size': lot_size,
                'amd_phase': market_structure.get('amd', {}).get('phase', 'UNKNOWN'),
                'session': market_structure.get('session', {}).get('name', 'UNKNOWN'),
                'regime': market_structure['regime']
            }
            
            # Create comment for MT5 (31 char limit)
            # Format: SETUP_REGIME_AMD
            setup_abbrev = setup['type'][:8].upper()
            regime_abbrev = market_structure['regime'][:8]
            amd_abbrev = market_structure.get('amd', {}).get('phase', 'UNK')[:7]
            comment = f"{setup_abbrev}_{regime_abbrev}_{amd_abbrev}"
            
            # Place order
            result = self.order_manager.place_order(
                symbol, lot_size, direction, stop_loss, take_profit, comment=comment
            )
            
            if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                self.last_trade_time[symbol] = datetime.now(timezone.utc)
                
                # Get the actual position to find ticket number
                time.sleep(0.5)  # Brief pause to ensure position is registered
                positions = mt5.positions_get(symbol=symbol)
                if positions:
                    # Find our position (most recent)
                    our_position = max(positions, key=lambda p: p.time)
                    
                    # Store full context
                    self.active_positions[our_position.ticket] = entry_context
                    
                    # Initialize exit manager context if available
                    if self.exit_manager:
                        self.exit_manager._initialize_position_with_context(
                            our_position, entry_context
                        )
            ...
            # --- FIX #3: Add traceback for better future debugging ---
            traceback.print_exc()
            return False, str(e)
        finally:
    ...
    def calculate_dynamic_stops(self, setup, market_structure):
        """
        Calculate stop and take profit levels based on setup type and market conditions.
        Returns: (stop_distance, take_profit_distance) for position sizing
        """
        atr = market_structure['atr']
        pip_size = self.market_data.get_pip_size()
        ...
        # STEP 1: Determine structural stop level with stop-hunt protection
        stop_hunt_buffer = atr * 0.3  # Base 30% ATR buffer
        
        # Adjust buffer based on session (more protection during high volatility)
        session_info = self.market_context.get_trading_session()
        if session_info['name'] in ['LONDON_NY_OVERLAP', 'LONDON_TOKYO_OVERLAP']:
            stop_hunt_buffer *= 1.2  # 20% more buffer during overlaps
        
        # Determine structural stop level based on setup type
        if setup['type'] == 'fibonacci' and 'zone' in setup:
            if setup['direction'] == 'buy':
                # Stop below the fib zone with hunt protection
                structural_stop = setup['zone'].lower - stop_hunt_buffer
            else:
                # Stop above the fib zone with hunt protection
                structural_stop = setup['zone'].upper + stop_hunt_buffer
                
        elif setup['type'] == 'ma_bounce' and 'stop_loss_level' in setup and setup['stop_loss_level'] is not None:
            # stop_loss_level is the swing low/high of the bounce pattern
            if setup['direction'] == 'buy':
                structural_stop = setup['stop_loss_level'] - stop_hunt_buffer
            else:
                structural_stop = setup['stop_loss_level'] + stop_hunt_buffer
                
        elif setup['type'] == 'shallow_pullback' and 'trend_low' in setup and 'trend_high' in setup:
            # Stop beyond the start of the impulse move
            if setup['direction'] == 'buy':
                structural_stop = setup['trend_low'] - stop_hunt_buffer
            else:
                structural_stop = setup['trend_high'] + stop_hunt_buffer
                
        elif setup['type'] == 'structure_break' and 'zone' in setup:
            # Stop if price returns through the broken level
            # Extra buffer here as these levels attract more stop hunting
            enhanced_buffer = stop_hunt_buffer * 1.5
            if setup['direction'] == 'buy':
                structural_stop = setup['zone'].lower - enhanced_buffer
            else:
                structural_stop = setup['zone'].upper + enhanced_buffer
                
        elif setup['type'] == 'range_extreme' and 'zone' in setup:
            # For range trades, stop goes beyond the range boundary
            # Extra buffer at range extremes as these are common stop hunt areas
            range_buffer = stop_hunt_buffer * 1.5
            
            # The zone center represents the range extreme
            # For upper extreme sells, stop above range high
            # For lower extreme buys, stop below range low
            if setup['direction'] == 'sell' and setup['extreme'] == 'upper':
                # Zone is near range high, stop above it
                structural_stop = setup['zone'].center + (setup['zone'].center - setup['zone'].lower) + range_buffer
            elif setup['direction'] == 'buy' and setup['extreme'] == 'lower':
                # Zone is near range low, stop below it
                structural_stop = setup['zone'].center - (setup['zone'].upper - setup['zone'].center) - range_buffer
            else:
                # Fallback (shouldn't happen)
                structural_stop = setup['zone'].center + (range_buffer * (1 if setup['direction'] == 'sell' else -1))
                
        elif setup['type'] == 'momentum':
            # For momentum trades in strong trends, the invalidation is less structural
            # and more about the trend losing steam. Use a tighter ATR-based approach
            # but with consideration for the recent move
            
            momentum_buffer = atr * 0.8  # Tighter stops for momentum trades
            
            # If we have information about the recent move, use it
            if 'pullback_depth' in setup and setup['pullback_depth'] < 0.382:
                # Very shallow pullback = even tighter stop
                momentum_buffer *= 0.8
            
            if setup['direction'] == 'buy':
                structural_stop = entry_price - momentum_buffer
            else:
                structural_stop = entry_price + momentum_buffer
                
        else:
            # Default fallback
            default_buffer = atr * 1.2
            if setup['direction'] == 'buy':
                structural_stop = entry_price - default_buffer
            else:
                structural_stop = entry_price + default_buffer
        
        # STEP 2: Calculate actual stop distance from entry
        stop_distance = abs(entry_price - structural_stop)
        
        # STEP 3: Validate the trade is still viable
        # Check if entry makes sense relative to stop
        if setup['direction'] == 'buy':
            if entry_price <= structural_stop:
                logging.warning(f"Buy entry {entry_price:.5f} is at/below stop {structural_stop:.5f}")
                return None, None
            # Additional check: if stop distance is > 3 ATR, entry is too late
            if stop_distance > atr * 3:
                logging.warning(f"Stop distance {stop_distance/pip_size:.1f} pips exceeds 3 ATR - entry too late")
                return None, None
        else:  # sell
            if entry_price >= structural_stop:
                logging.warning(f"Sell entry {entry_price:.5f} is at/above stop {structural_stop:.5f}")
                return None, None
            if stop_distance > atr * 3:
                logging.warning(f"Stop distance {stop_distance/pip_size:.1f} pips exceeds 3 ATR - entry too late")
                return None, None
        
        # STEP 4: Adjust for market regime (but not too much)
        regime_adjustment = 1.0
        if market_structure['regime'] == 'STRONG_TREND':
            regime_adjustment = 0.9  # Slightly tighter in strong trends
        elif market_structure['regime'] == 'VOLATILE_EXPANSION':
            regime_adjustment = 1.3  # Wider in volatile conditions
        elif market_structure['regime'] == 'RANGING':
            regime_adjustment = 1.1  # Slightly wider in ranges (more noise)
        
        # Apply regime adjustment to stop distance
        stop_distance *= regime_adjustment
        
        # STEP 5: Ensure minimum stop distance
        min_stop = 15 * pip_size  # Minimum 15 pips
        stop_distance = max(stop_distance, min_stop)
        
        # STEP 6: Calculate take profit based on R:R and setup quality
        base_rr = 2.0  # Base risk-reward ratio
        
        # Adjust R:R based on setup type and market conditions
        if setup['type'] == 'momentum' and market_structure['regime'] == 'STRONG_TREND':
            base_rr = 3.0  # Higher R:R for momentum in strong trends
        elif setup['type'] == 'range_extreme':
            # For range trades, conservative R:R as we're trading against the extremes
            base_rr = 1.5
            # But if the range is wide enough, we can aim for more
            if stop_distance < atr * 1.5:  # If stop is reasonably tight
                base_rr = 2.0
        elif setup['type'] == 'structure_break' and setup.get('retest', False):
            base_rr = 2.5  # Better R:R on retested breaks
        elif setup['type'] == 'shallow_pullback':
            # Shallow pullbacks in strong trends can run far
            base_rr = 2.5
        
        # Adjust R:R based on setup score (higher confidence = higher target)
        if setup.get('score', 0) > 0.85:
            base_rr *= 1.2
        elif setup.get('score', 0) < 0.65:
            base_rr *= 0.8
        
        take_profit_distance = stop_distance * base_rr
        
        # Log the calculation for debugging
        logging.debug(f"Stop calculation for {setup['type']} {setup['direction']}:")
        logging.debug(f"  Entry: {entry_price:.5f}, Structural Stop: {structural_stop:.5f}")
        logging.debug(f"  Stop Distance: {stop_distance/pip_size:.1f} pips, TP Distance: {take_profit_distance/pip_size:.1f} pips")
        logging.debug(f"  R:R Ratio: {base_rr:.1f}, Regime Adjustment: {regime_adjustment:.1f}")
...
class OrderManager:
    ...
    def place_order(self, symbol, lots_size, order_type, stop_loss, take_profit, comment=""):
...
def main():
    ...
    SYMBOLS = ['AUDUSD', 'CHFJPY', 'EURUSD', 'GBPUSD', 'USDCAD', 'USDCHF', 'USDJPY', 
               'EURCAD', 'GBPJPY', 'AUDCHF', 'AUDCAD', 'AUDJPY', 'EURJPY', 
               'AUDNZD', 'CADCHF', 'GBPAUD', 'GBPCAD', 'NZDUSD']
    ...
    # Track trade managers for each symbol
    trade_managers = {}
    ...
    if existing_positions:
        logging.info(f"Found {len(existing_positions)} existing positions on startup")
        for position in existing_positions:
            # Initialize exit context from comment/defaults
            exit_manager._initialize_position_context(position)
    ...
    try:
        while True:
            ...
            for symbol in SYMBOLS:
                try:
                    # Initialize trade manager if needed
                    if symbol not in trade_managers:
                        symbol_market_data = MarketData(symbol, TIMEFRAMES)
                        trade_managers[symbol] = AdaptiveTradeManager(
                            client, symbol_market_data, market_context, correlation_manager
                        )
                        # Link exit manager
                        trade_managers[symbol].set_exit_manager(exit_manager)
            ...
            # === EXIT MANAGEMENT PHASE ===
            positions = mt5.positions_get()
            if positions:
                logging.info(f"\n[Exit Management] Checking {len(positions)} positions...")
                
                exit_checks_performed = 0
                exits_triggered = 0
                ...
                for position in positions:
                    try:
                        ...
                        market_data_dict = all_market_data.get(symbol, {})
                        
                        if not market_data_dict:
                            logging.warning(f"No market data available for position {symbol}")
                            continue
                        
                        # Ensure we have all timeframes
                        if not all(tf in market_data_dict for tf in TIMEFRAMES):
                            logging.warning(f"Incomplete market data for {symbol}")
                            continue
                        
                        # Get current market structure if we have a trade manager
                        if symbol in trade_managers and trade_managers[symbol].exit_manager:
                            exit_signals = trade_managers[symbol].exit_manager.analyze_position(
                                position, market_data_dict, TIMEFRAMES
                            )
                        
                        # Analyze position for exit signals
                        exit_signals = exit_manager.analyze_position(
                            position, market_data_dict, TIMEFRAMES
                        )
                        
                        exit_checks_performed += 1
                        
                        if exit_signals:
                            # Log all signals found
                            logging.debug(f"[{symbol}] Found {len(exit_signals)} exit signals:")
                            for sig in exit_signals:
                                logging.debug(f"  - {sig.action}: {sig.reason} (urgency: {sig.urgency})")
                            
                            # Sort by urgency and priority
                            immediate = [s for s in exit_signals if s.urgency == 'immediate']
                            normal = [s for s in exit_signals if s.urgency == 'normal']
                            
                            # Execute most urgent signal
                            signal_to_execute = None
                            if immediate:
                                # Take first immediate signal
                                signal_to_execute = immediate[0]
                            elif normal:
                                # Priority: full_exit > partial_exit > trail_stop > move_to_breakeven
                                priority_map = {
                                    'full_exit': 1,
                                    'partial_exit': 2,
                                    'trail_stop': 3,
                                ...
                                normal.sort(key=lambda s: priority_map.get(s.action, 5))
                                signal_to_execute = normal[0]
                            
                            if signal_to_execute:
                                success = exit_manager.execute_exit_signal(
                                    signal_to_execute, position
                                )
                                if success:
                                    exits_triggered += 1
                                    logging.info(f"[{position.symbol}] Exit executed: "
                                               f"{signal_to_execute.action} - "
                                               f"{signal_to_execute.reason}")
                                    
                                    # Track exit reasons
                                    reason_key = signal_to_execute.reason[:30]  # Truncate for grouping
                                    performance['exits_by_reason'][reason_key] = \
                                        performance['exits_by_reason'].get(reason_key, 0) + 1
                                    performance['total_exits'] += 1
                                    
                                    # If position was fully closed, clean up trade manager tracking
                                    if signal_to_execute.action == 'full_exit' and symbol in trade_managers:
                                        if hasattr(trade_managers[symbol], 'active_positions'):
                                            trade_managers[symbol].active_positions.pop(position.ticket, None)
                                else:
                                    logging.warning(f"Failed to execute {signal_to_execute.action} "
                ...
                if exit_checks_performed > 0:
                    logging.info(f"Exit checks completed: {exit_checks_performed} positions, "
                               f"{exits_triggered} actions taken")
            
            # Monitor positions summary
            positions = mt5.positions_get()
            if positions:
                ...
                for pos in positions:
                    r_multiple = 0
                    if pos.ticket in exit_manager.position_contexts:
                        context = exit_manager.position_contexts[pos.ticket]
                        if pos.type == 0:  # Buy
                            initial_risk = pos.price_open - pos.sl
                            current_profit = pos.price_current - pos.price_open
                        else:  # Sell
                            initial_risk = pos.sl - pos.price_open
                            current_profit = pos.price_open - pos.price_current
                        
                        if initial_risk > 0:
                            r_multiple = current_profit / initial_risk
                            
                        logging.info(f"  {pos.symbol}: {context.entry_setup_type} "
                                   f"({context.exit_strategy}) "
                                   f"R={r_multiple:.2f} P/L=${pos.profit:.2f}")
                    else:
                        logging.info(f"  {pos.symbol}: P/L=${pos.profit:.2f}")
            ...
            if time.time() - performance['last_update'] > 1800:
                ...
                # Exit statistics
                if performance['total_exits'] > 0:
                    logging.info(f"\nExit Statistics:")
                    logging.info(f"Total Exits: {performance['total_exits']}")
                    logging.info("Exits by Reason:")
                    sorted_exits = sorted(performance['exits_by_reason'].items(), 
                                        key=lambda x: x[1], reverse=True)
                    for reason, count in sorted_exits[:5]:  # Top 5 reasons
                        pct = (count / performance['total_exits']) * 100
                        logging.info(f"  {reason}: {count} ({pct:.1f}%)")
                
                # Get detailed exit statistics from exit manager
                exit_stats = exit_manager.get_exit_statistics()
                if exit_stats:
                    logging.info("\nExit Performance by Type:")
                    for reason, stats in exit_stats.items():
                        logging.info(f"  {reason}: {stats['count']} exits, "
                                   f"avg {stats['avg_r_multiple']}R")
            ...
            if sleep_time > 0:
                if signals_found > 0 or exits_triggered > 0:
                    logging.info(f"\nExecuted {signals_found} trades, {exits_triggered} exits. "
                               f"Next scan in {sleep_time:.0f} seconds...")
                else:
                    logging.info(f"\nNo new setups or exits. Next scan in {sleep_time:.0f} seconds...")
        ...
        if performance['total_trades'] > 0:
            logging.info(f"\nTotal Trades Executed: {performance['total_trades']}")
            logging.info("\nMost Active Setups:")
        ...
        if performance['total_exits'] > 0:
            logging.info(f"\nTotal Exits: {performance['total_exits']}")
            logging.info("\nTop Exit Reasons:")
            sorted_exits = sorted(performance['exits_by_reason'].items(), 
                                key=lambda x: x[1], reverse=True)
            for reason, count in sorted_exits[:3]:
                logging.info(f"  {reason}: {count} exits")
        
        # Final exit statistics
        exit_stats = exit_manager.get_exit_statistics()
        if exit_stats:
            logging.info("\nExit Performance Summary:")
            total_r = sum(s['total_r'] for s in exit_stats.values())
            total_exits = sum(s['count'] for s in exit_stats.values())
            if total_exits > 0:
                avg_r = total_r / total_exits
                logging.info(f"  Average R-Multiple: {avg_r:.2f}")
                logging.info(f"  Total R Generated: {total_r:.2f}")
